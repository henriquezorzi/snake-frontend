...
export default function Ranking() {
  ...
  const fetchScores = async () => {
    try {
      ...
      let response;
      try {
        response = await axios.get(`http://localhost:4000/ranking`);
      } catch (rankingError) {
        // Se não existir endpoint de ranking, busca todos os jogadores
        response = await axios.get(`http://localhost:4000//jogadores`);
      }
  ...
  return (
    <>
      <Link
        href="/"
        className="absolute top-3 left-3 inline-flex items-center text-xl text-white hover:text-yellow-300 transition-colors duration-200 z-50"
      ...
      <div className="relative min-h-screen bg-[url('/bg-clouds.png')] bg-cover flex flex-col items-center justify-center p-6">
